...
class ClubController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        $formClub->handleRequest($request); // bech torbot qol champ mel formulare bel attrbut mta3 l club mta3ou ( npût lbelle <-> lbelle ...)

        $club->setCreatedBy($user); // club crée par le user connecté
        $club->setEtat('EnAttente');

        if ($formClub->isSubmitted() && $formClub->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->persist($club); // ajouter dans la base de donnés
            $em->flush();
            return $this->redirectToRoute('club_index'); // redrecton lel pâge d'accuel
        }
    ...
    /**
     *
     * @Route("/rechercheClubByTypeKeyword", name="clu  b_type_recherche")
     * @Method({"GET", "POST"})
     */
